  ...
  return <main className="min-h-screen bg-[#f8fafc]">
    ...
    <section className="mx-auto max-w-5xl px-6 py-12">
      {comingSoon ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-7">
        ...
        <CourseProgress courseSlug={course.slug} lessonCount={course.lessons.length} labCount={labs.filter(lab => lab.status === "available").length} firstLesson={course.lessons[0]?.slug ?? ""}/><CertificateCard courseSlug={course.slug} lessonCount={course.lessons.length} labCount={labs.filter(lab => lab.status === "available").length}/>
      </div>}
