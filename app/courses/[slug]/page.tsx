...
export default async function CoursePage({params}:{params:Promise<{slug:string}>}) {
  const {slug}=await params; const course=await getCourseFromDb(slug); if(!course) notFound();
  return <main className="min-h-screen bg-[#f8fafc]">
    <header className="bg-[#0b1f3a] text-white"><div className="mx-auto max-w-5xl px-6 py-14">
      ...
      <div className="mt-10 max-w-3xl"><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-blue-200">{course.level}</span>
      ...
      <div className="mt-7 flex flex-wrap gap-5 text-sm font-semibold text-slate-300"><span className="inline-flex items-center gap-2"><PlayCircle size={16}/> {course.lessons.length} lessons</span><span className="inline-flex items-center gap-2"><Clock3 size={16}/> {course.duration}</span></div></div>
    </div></header>
    <section className="mx-auto max-w-5xl px-6 py-12"><div className="rounded-2xl border border-slate-200 bg-white p-7">
