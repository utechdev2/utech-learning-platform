    ...
    <main className="min-h-screen bg-[#f8fafc]">
      ...
      <section className="mx-auto max-w-7xl px-6 py-14">
        <div className="grid gap-6 md:grid-cols-2">
          {courses.map(course => {
            const Icon = icons[course.slug as keyof typeof icons];
            return <article key={course.slug} className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
              ...
              <h2 className="mt-7 text-2xl font-black text-[#0b1f3a]">{course.title}</h2>
              <p className="mt-3 max-w-xl leading-7 text-slate-500">{course.description}</p>
