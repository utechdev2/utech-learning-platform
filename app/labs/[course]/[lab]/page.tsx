...
const starters: Record<string, { runtime: "python" | "clangpp" | "sqlite"; code: string }> = {
  "python-function-lab": { runtime: "python", code: "def greet(name):\n    return f\"Hello, {name}! Welcome to UTECH.\"\n\nname = input(\"Enter your name: \")\nprint(greet(name))\n" },
  "cpp-coding-lab": { runtime: "clangpp", code: "#include <iostream>\n#include <vector>\n#include <numeric>\n#include <iomanip>\nusing namespace std;\n\ndouble calculateAverage(const vector<int>& scores) {\n    int total = 0;\n    for (int score : scores) {\n        total += score;\n    }\n    return static_cast<double>(total) / scores.size();\n}\n\nchar getLetterGrade(double average) {\n    if (average >= 90) return 'A';\n    if (average >= 80) return 'B';\n    if (average >= 70) return 'C';\n    if (average >= 60) return 'D';\n    return 'F';\n}\n\nint main() {\n    vector<int> scores;\n\n    for (int i = 0; i < 5; ++i) {\n        int score;\n        cout << \"Enter score \" << i + 1 << \" (0-100): \";\n        cin >> score;\n\n        while (score < 0 || score > 100) {\n            cout << \"Invalid score. Enter a value from 0 to 100: \";\n            cin >> score;\n        }\n\n        scores.push_back(score);\n    }\n\n    double average = calculateAverage(scores);\n    char grade = getLetterGrade(average);\n\n    cout << fixed << setprecision(2);\n    cout << \"Average: \" << average << endl;\n    cout << \"Grade: \" << grade << endl;\n    return 0;\n}\n" },
  "sql-database-lab": { runtime: "sqlite", code: "CREATE TABLE students (\n  id INTEGER PRIMARY KEY,\n  name TEXT NOT NULL,\n  age INTEGER NOT NULL\n);\n\nINSERT INTO students (name, age) VALUES\n  ('Oscar', 19),\n  ('Ama', 20),\n  ('Kojo', 21);\n\nSELECT * FROM students ORDER BY age;\n" },
};

const challenges: Record<string, { goal: string; tasks: string[]; success: string[] }> = {
  "python-function-lab": { goal: "Turn the starter program into a reusable greeting tool.", tasks: ["Keep the greet(name) function and make it return a personalized message.", "Ask the user for a name with input().", "Call the function and print its returned value.", "Run the program with at least two different names."], success: ["The program runs without an error.", "The output changes when the name changes.", "The greeting logic lives inside the function."] },
  "cpp-coding-lab": { goal: "Build a C++ grade calculator that safely accepts five scores.", tasks: ["Read exactly five scores from the user with cin and store them in a vector.", "Reject scores below 0 or above 100 and ask for the score again.", "Use at least one custom function to calculate the average and another to determine the letter grade.", "Print the average and a letter grade (A, B, C, D or F)."], success: ["The C++ program compiles and runs with user input.", "Invalid scores are rejected instead of being accepted.", "The average is calculated from the entered vector values.", "A letter grade is displayed alongside the numerical average."] },
  "sql-database-lab": { goal: "Build and query a small student database.", tasks: ["Create the students table with an id, name and age.", "Insert at least three student records.", "Write a SELECT query that sorts students by age.", "Add one more student and verify the result changes."], success: ["The SQL executes without errors.", "The query returns the student records.", "The returned rows are ordered by age."] },
};
        ...
        {challenge && (
          <section className="mb-7 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-2xl border border-blue-200 bg-[#0b1f3a] p-7 text-white shadow-sm">
              ...
              <div className="mt-6 space-y-3">{challenge.tasks.map((task, index) => <div key={task} className="flex gap-3 rounded-xl border border-white/10 bg-white/5 p-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/10 text-xs font-black text-blue-200">{index + 1}</span><p className="text-sm leading-6 text-slate-200">{task}</p></div>)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
              ...
              <div className="mt-5 space-y-3">{challenge.success.map((item) => <div key={item} className="flex gap-3 rounded-xl bg-emerald-50 p-3 text-sm font-semibold leading-6 text-emerald-950"><CheckCircle2 className="mt-0.5 shrink-0 text-emerald-600" size={17} /><span>{item}</span></div>)}</div>
            </div>
          </section>
        )}
