...
function SignInForm() {
  ...
  return (
    <form onSubmit={submit} className="mt-7 space-y-4">
      ...
      <input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500" />
      <button className="w-full rounded-xl bg-[#0b1f3a] px-5 py-3 font-bold text-white hover:bg-[#122d52]">Sign in</button>
    ...
    <main className="min-h-screen bg-[#f8fafc] px-6 py-12">
      <div className="mx-auto max-w-md">
        ...
        <div className="mt-10 rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <Suspense fallback={<p className="mt-7 text-sm text-slate-500">Loading sign-in form...</p>}>
            <SignInForm />
          </Suspense>
          <p className="mt-6 text-sm text-slate-500">New to UTECH?{" "}<Link href="/auth/sign-up" className="font-bold text-[#155eef]">Create an account</Link></p>
        </div>
      </div>
    </main>
  );
